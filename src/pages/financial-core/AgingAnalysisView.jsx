...
import { getAgeingAnalysisData } from "../../apiCalls/salesAPI/salesAPICall";
import { GET_AGING_ANALYSIS } from "../../services/endPoints";
...
export default function AgingAnalysisView({ isDarkMode }) {
  ...
  const filteredCustomers = useMemo(() => {
    return customers.filter((customer) => {
      if (selectedCustomer !== "All" && customer.name !== selectedCustomer)
        return false;
      if (selectedCurrency !== "All" && customer.currency !== selectedCurrency)
        return false;
      if (
        selectedLegalEntity !== "All" &&
        customer.legalEntity !== selectedLegalEntity
      )
        return false;
      if (
        selectedSalesOwner !== "All" &&
        customer.salesOwner !== selectedSalesOwner
      )
        return false;
      return true;
    });
  ...
  const totals = useMemo(() => {
    const calculateTotal = filteredCustomers.reduce(
      (acc, customer) => {
        return {
  ...
  const handleGetAgeingAnalysisData = async () => {
    const response = await getAgeingAnalysisData(GET_AGING_ANALYSIS);
        ...
        <div className="aging-section-header">
          <div className="aging-title-row">
            ...
            <button
              ...
              title="Information about aging analysis"
            >
              <svg
                width="16"
                height="16"
          ...
          <p className="aging-subtitle">
            Track overdue receivables by aging bucket
          </p>
        </div>
          ...
          <table className="aging-table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td style={{ color: "#00A63E", fontWeight: "bold" }}>
                  {totals.current}
                </td>
